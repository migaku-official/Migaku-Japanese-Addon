...
UEManager = UserExceptionManager(mw, addon_path)  ### feed reference to UEManager to AccentExporter it can be used to try and replace through
...
def loadCollectionArray(self = None):
    global colArray
    colArray = {}
    # showInfo('here')
...
setupGuiMenu()
AnkiQt.showProfileManager = wrap(AnkiQt.showProfileManager, loadCollectionArray, 'before')


def copyIcons():
    src = join(addon_path, 'icons')
    for filename in os.listdir(src):
        filesrc = join(addon_path, 'icons', filename)
        path = join(os.getcwd(), 'web', 'imgs', filename)
        if not exists(path): 
            copyfile(src, path)
            
# copyIcons()
# addHook("unloadProfile", reloadCollectionArray)
